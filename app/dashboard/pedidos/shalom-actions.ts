...
const REQUIRED_SHALOM_ADVANCE = 30;
...
async function validatedAdvance(
  admin: ReturnType<typeof createAdminSupabase>,
  orderId: string,
): Promise<number> {
  const { data } = await admin
  ...
  return ((data ?? []) as { amount: number | string | null }[]).reduce(
...
export async function loadShalomDraft(
  ...
  const advance = await validatedAdvance(admin, orderId);
  const advanceReady = advance >= REQUIRED_SHALOM_ADVANCE;
  if (!advanceReady) {
    blockers.push(
      `Se requieren S/ ${REQUIRED_SHALOM_ADVANCE.toFixed(2)} de adelanto validado antes de crear la guía Shalom. Validado: S/ ${advance.toFixed(2)}.`,
    );
  }
  ...
  return {
    draft: {
      ...
      requiredAdvance: REQUIRED_SHALOM_ADVANCE,
      advanceReady,
...
export async function createShalomGuide(
  ...
  // El adelanto de S/30 es una regla dura: también se revalida al enviar para
  // evitar que una pestaña abierta salte un pago borrado o aún no validado.
  const advance = await validatedAdvance(admin, orderId);
  if (advance < REQUIRED_SHALOM_ADVANCE) {
    return {
      error: `No se puede crear la guía: se requieren S/ ${REQUIRED_SHALOM_ADVANCE.toFixed(2)} de adelanto validado y actualmente hay S/ ${advance.toFixed(2)}.`,
    };
  }
