...
import {
  ...
  spinOffFenixGuide,
  swaypGuideForReprogram,
  swaypSinVinculo,
  type DirectGuideOrderRecord,
...
export async function reprogramCancelledShipmentException(
  ...
  // Las rejas y el alta viven en `reenviarGuiaAnulada`, compartida con el
  // agente de voz (MOM §11.8): cobertura y stock de hoy, número emitido por
  // Swayp —sin él no se registra nada— y el codbar en `spinOffFenixGuide`.
  const r = await reenviarGuiaAnulada(createAdminSupabase(), ctx, shipmentId, input);
