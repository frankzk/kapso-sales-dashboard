...
export function RiderPayPanel({ routeId, initialDetail }: { routeId: string; initialDetail?: RiderPayDetail }) {
  ...
  if (loading) return <section aria-busy="true" aria-label="Cargando liquidación diaria" className="rounded-xl border border-slate-200 p-4"><p>Cargando ganancia y saldo del motorizado…</p></section>;
  ...
  return <section aria-label="Liquidación diaria del motorizado" className="space-y-5 rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
    <header className="space-y-1">
      ...
      <p className="text-sm font-medium">{detail.approved ? `Cálculo aprobado el ${new Date(detail.approved.at).toLocaleString("es-PE", { timeZone: "America/Lima" })}` : "Borrador: revisa antes de aprobar"}</p>
    </header>
    ...
    <div className="flex flex-wrap items-baseline justify-between gap-2 rounded-lg bg-slate-50 p-4">
      <p className="font-semibold">{s.net_cash === null ? "Saldo por calcular" : s.net_cash > 0 ? "El motorizado debe entregar" : s.net_cash < 0 ? "Grupo GF debe pagarle" : "Sin efectivo pendiente entre ambos"}</p>
      <p className="text-xl font-semibold tabular-nums">{money(s.net_cash === null ? null : Math.abs(s.net_cash))}</p>
      <p className="w-full text-xs text-slate-600">Efectivo menos ganancia base y adicionales. No incluye Yape/POS como efectivo del motorizado. Aprobar este cálculo no valida ingresos bancarios ni registra un pago o depósito.</p>
    </div>
    ...
    {detail.canConfigure && <details className="border-t border-slate-200 pt-3"><summary className="min-h-12 cursor-pointer font-medium">Configurar tarifa de {s.rider_name}</summary>
    ...
    {!detail.approved && detail.canApprove && <>
      <details className="border-t border-slate-200 pt-3"><summary className="min-h-12 cursor-pointer font-medium">Aprobar adicional por espera, retorno u otra excepción</summary>
