...
export function CourierRouteNav({ current }: { current: "cajas" | "reparto" }) {
  return <header className="space-y-3">
    ...
    <nav aria-label="Secciones de Grupo GF Courier" className="grid grid-cols-5 gap-1 border-b border-slate-200 text-center text-sm">
      {[["Despacho", "dispatch"], ["Disponibles", "available"], ["Tomados", "preparation"], ["Rutas", "routes"], ["Tarifario", "tariffs"]].map(([label, tab]) =>
