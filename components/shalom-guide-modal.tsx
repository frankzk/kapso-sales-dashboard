  ...
  useEffect(() => {
    ...
    void loadShalomProducts(storeId).then((res) => {
      ...
      setProductId((current) => {
        if (current && res.products.some((p) => p.id === current)) return current;
        const xxs = res.products.find((p) =>
          p.title.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().includes("caja paquete xxs"),
        );
        return xxs?.id ?? res.products[0]?.id ?? null;
      });
    });
      ...
      >
        ...
        {draft && (
          <div className="space-y-4 p-5">
            ...
            {!draft.advanceReady && !created ? (
              <PickupKeyPanel
                orderId={orderId}
                onChanged={() => {
                  void loadShalomDraft(orderId).then((res) => {
                    if ("draft" in res) setDraft(res.draft);
                  });
                  onCreated();
                }}
              />
            ...
            {created ? (
              ...
              <>
                ...
                <p className="rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">
                  La clave de recojo se genera automáticamente, queda cifrada y se entrega al cliente
                  desde el panel de pagos cuando el cobro completo esté validado.
                </p>
