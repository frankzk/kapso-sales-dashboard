...
-- ---- 0189 ----
-- 0189_gf_return_rejected.sql — «Recibir en oficina» de un pedido RECHAZADO
-- (MOM §29.13, 22-09-2026).
--
-- 0188 devolvía todo «No entregado» a «por asignar». Un rechazo en puerta no se
-- reprograma: la venta terminó. Aquí, si la parada dice «rechazado», la salida
-- queda devuelta (`custody_state = devuelto`, `returned_at`) y la solicitud del
-- courier se cancela; el pedido pasa a «Por cerrar · Devolución pendiente de
-- inventario». El resto de motivos sigue igual que en 0188.
--
...
begin
  ...
  if v_stop.outcome_reason = 'rechazado' then
    -- El cliente rechazó el pedido: no se reprograma. La salida queda devuelta
    -- (el paquete ya está en la oficina) y el servicio del courier se cierra.
    update shipments
       set custody_state = 'devuelto', returned_at = now(), custody_transferred_at = null, custody_transferred_by = null
     where id = v_item.shipment_id;
    update logistics_requests
       set status = 'cancelled', observation = left(v_reason, 300)
     where shipment_id = v_item.shipment_id and status in ('scheduled', 'accepted');
  else
    update shipments
       set custody_state = 'empresa', custody_transferred_at = null, custody_transferred_by = null, dispatched_at = null
     where id = v_item.shipment_id;
  ...
  if v_shipment.order_id is not null then
    ...
    values (v_shipment.store_id, v_shipment.order_id, 'returned_to_office', p_actor, 'dispatch', 'propio',
            v_shipment.guide_code, v_item.shipment_id, v_stop.outcome_reason,
            v_reason || case when v_stop.outcome_reason = 'rechazado' then '. Rechazado por el cliente: queda devuelto, no se reprograma.' else '. Vuelve a «por asignar».' end,
            jsonb_build_object('manifest_id', v_item.manifest_id, 'rider_id', v_manifest.rider_id,
                               'route_date', v_manifest.route_date, 'stop_id', v_stop.id));
  end if;
  return v_shipment.order_id;
