...
| 2 · ¿Funciona la saliente? | 22-09-2026 | **Zadarma acepta el callback y sí marca, pero al número equivocado.** El historial (`/v1/statistics/`) muestra cada intento como `to: 5151930555309`, `disposition: failed`: la cuenta antepone el código de Perú y el `51` que mandamos queda duplicado. **El `to` va sin `51`** (`to=930555309`). Dos hallazgos más: el caller ID por defecto del SIP 499999 es un número de EE. UU. (`12027734798`), que una clienta peruana difícilmente contesta; y la cuenta la comparte otra operación que llama a Costa Rica desde la extensión 103, así que el caller ID se cambia por extensión, no en el SIP. Pendiente: repetir con `to` sin `51` y anotar silencio y «Caller» en xAI. |
